...
namespace riscv_tlm::peripherals {
    ...
    void Timer::b_transport(tlm::tlm_generic_payload &trans,
        ...
        std::uint64_t aux_value = 0;


        if (cmd == tlm::TLM_WRITE_COMMAND) {
            unsigned int copy_len = len > 8 ? 8 : len;
            memcpy(&aux_value, ptr, copy_len);
            switch (addr) {
                case TIMER_MEMORY_ADDRESS_LO:
                    m_mtime.range(31, 0) = aux_value & 0xFFFFFFFF;
                    break;
                case TIMER_MEMORY_ADDRESS_HI:
                    m_mtime.range(63, 32) = aux_value & 0xFFFFFFFF;
                    break;
                case TIMERCMP_MEMORY_ADDRESS_LO:
                    m_mtimecmp.range(31, 0) = aux_value & 0xFFFFFFFF;
                    break;
                case TIMERCMP_MEMORY_ADDRESS_HI:
                    m_mtimecmp.range(63, 32) = aux_value & 0xFFFFFFFF;
