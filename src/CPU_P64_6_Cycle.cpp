...
CPURV64P6_Cycle::CPURV64P6_Cycle(sc_core::sc_module_name const& name,
    ...
    // GP = 0x10FF3: places buffer (gp-2035) at 0x10800 — 4-byte aligned, above .text end (0x10722)
...
void CPURV64P6_Cycle::cycle_thread() {
    ...
    while (true) {
        ...
        if (wfi_stall) {
            ...
            bool csr_irq_pending = (csr.mip & csr.mie) != 0 ||
                                   timer_irq_in.read() ||
                                   msip_irq_in.read()  ||
                                   ext_irq_in.read();
            if (!csr_irq_pending) {
        ...
        // When stall_ex is set (partial store-buffer overlap), keep ALL latches
        // unchanged so the load retries and upstream stages freeze.
        if (stall_ex) {
        ...
        if (timer_irq_in.read()) csr.mip |= (1ULL << 7);  // MTIP
        else                      csr.mip &= ~(1ULL << 7);
        if (msip_irq_in.read())   csr.mip |= (1ULL << 3);  // MSIP
        else                      csr.mip &= ~(1ULL << 3);
        if (ext_irq_in.read())    csr.mip |= (1ULL << 9);  // SEIP (supervisor external)
        else                      csr.mip &= ~(1ULL << 9);
        ...
        // Must be captured before PCGen_stage() clears it so the trace records correctly.
        ...
        // NOTE: PCGen_stage() resets flush_pipeline to false — use this_cycle_flush for trace.
        PCGen_stage();
        ...
        if (stats.cycles > 100 && 
            ...
            std::cout << "  mcause: 0x" << std::hex << csr.mcause
                      << " | mepc: 0x" << csr.mepc
                      << " | mtval: 0x" << csr.mtval
                      << " | mstatus: 0x" << csr.mstatus << std::dec << std::endl;
            printStats();
            sc_core::sc_stop();
            break;
        }
...
void CPURV64P6_Cycle::Fetch_stage() {
    ...
    // While counting down, hold the PC (stall_pcgen) and inject bubbles downstream.
    // When the counter reaches 0 we fall through and re-fetch (the cache line is now warm).
    ...
    uint64_t fetch_pa = current_pc;
    {
        ...
        if (tr.fault) {
            ...
            uint64_t tvec = csr.take_exception(tr.fault_cause, current_pc, current_pc);
            pc_redirect_target = tvec; pc_redirect_valid = true;
            flush_pipeline = true; trap_pending = true;
            fetch_id_next.valid = false;
            return;
        }
        ...
        if (!csr.pmp_check(fetch_pa, 4, false, true)) {
            uint64_t tvec = csr.take_exception(CAUSE::INSTR_ACCESS, current_pc, current_pc);
            pc_redirect_target = tvec; pc_redirect_valid = true;
            flush_pipeline = true; trap_pending = true;
            fetch_id_next.valid = false;
            return;
    ...
    if (is_c && !pcgen_fetch_reg.predicted_taken) next_pc -= 2;
}
...
void CPURV64P6_Cycle::ID_stage() {
    ...
    if (fetch_id_reg.is_compressed) {
        ...
        switch (c_deco) {
            ...
            case OP_C_ADDI4SPN: {
                ...
                id_issue_next.opcode = 0x13; id_issue_next.funct3 = 0;
                id_issue_next.rd  = c_inst->get_rdp();
                id_issue_next.rs1 = 2; id_issue_next.rs2 = 0;
                id_issue_next.imm = addi4spn_imm;
                break;
            }
            case OP_C_LW:
                // LW rd', offset(rs1')
                id_issue_next.opcode = 0x03; id_issue_next.funct3 = 2;
                id_issue_next.rd  = c_inst->get_rdp();
                id_issue_next.rs1 = c_inst->get_rs1p(); id_issue_next.rs2 = 0;
                id_issue_next.imm = static_cast<int64_t>(c_inst->get_imm_L());
                break;
            case OP_C_LD:
                // LD rd', offset(rs1')  [RV64; also covers C.FLW in RV64 decoded as LD]
                id_issue_next.opcode = 0x03; id_issue_next.funct3 = 3;
                id_issue_next.rd  = c_inst->get_rdp();
                id_issue_next.rs1 = c_inst->get_rs1p(); id_issue_next.rs2 = 0;
            ...
            case OP_C_SW:
                // SW rs2', offset(rs1')
                id_issue_next.opcode = 0x23; id_issue_next.funct3 = 2;
                id_issue_next.rd  = 0;
            ...
            case OP_C_SD:
                ...
                id_issue_next.opcode = 0x23; id_issue_next.funct3 = 3;
                id_issue_next.rd  = 0;
                id_issue_next.rs1 = c_inst->get_rs1p();
                id_issue_next.rs2 = c_inst->get_rs2p();
                id_issue_next.imm = static_cast<int64_t>(c_inst->get_imm_CL());
            ...
            case OP_C_FLD:
                // FLD rd', offset(rs1')
                id_issue_next.opcode = 0x07; id_issue_next.funct3 = 3;
                id_issue_next.rd  = c_inst->get_rdp();
                id_issue_next.rs1 = c_inst->get_rs1p(); id_issue_next.rs2 = 0;
                id_issue_next.imm = static_cast<int64_t>(c_inst->get_imm_CL());
                break;
            case OP_C_FSD:
                // FSD rs2', offset(rs1')
                id_issue_next.opcode = 0x27; id_issue_next.funct3 = 3;
                id_issue_next.rd  = 0;
                id_issue_next.rs1 = c_inst->get_rs1p();
                id_issue_next.rs2 = c_inst->get_rs2p();
                id_issue_next.imm = static_cast<int64_t>(c_inst->get_imm_CL());
            ...
            case OP_C_NOP:
                // NOP → ADDI x0, x0, 0
                id_issue_next.opcode = 0x13; id_issue_next.funct3 = 0;
                id_issue_next.rd = 0; id_issue_next.rs1 = 0; id_issue_next.rs2 = 0;
                id_issue_next.imm = 0;
                break;
            case OP_C_ADDI:
                // ADDI rd, rd, nzimm
            ...
            case OP_C_ADDIW:
                ...
                id_issue_next.opcode = 0x1B; id_issue_next.funct3 = 0;
            ...
            case OP_C_LI:
                ...
                id_issue_next.opcode = 0x13; id_issue_next.funct3 = 0;
                id_issue_next.rd  = c_inst->get_rs1();
                id_issue_next.rs1 = 0; id_issue_next.rs2 = 0;
                id_issue_next.imm = static_cast<int64_t>(c_inst->get_imm_ADDI());
                break;
            case OP_C_ADDI16SP: {
                ...
                if (rd_f == 2) {
                    // ADDI x2, x2, nzimm
                    id_issue_next.opcode = 0x13; id_issue_next.funct3 = 0;
                    id_issue_next.rd = 2; id_issue_next.rs1 = 2; id_issue_next.rs2 = 0;
                    id_issue_next.imm = static_cast<int64_t>(
                        static_cast<int32_t>(c_inst->get_imm_ADDI16SP()));
                } else {
                    // LUI rd, nzimm
                    id_issue_next.opcode = 0x37;
                    id_issue_next.rd = rd_f; id_issue_next.rs1 = 0; id_issue_next.rs2 = 0;
            ...
            case OP_C_SRLI: {
                ...
                id_issue_next.opcode = 0x13; id_issue_next.funct3 = 5;
            ...
            case OP_C_SRAI: {
                // SRAI rd', rd', shamt  — imm[10]=1 signals arithmetic shift in 64-bit SRLI/SRAI
                uint32_t shamt = (((ci >> 12) & 1) << 5) | ((ci >> 2) & 0x1F);
                id_issue_next.opcode = 0x13; id_issue_next.funct3 = 5;
                id_issue_next.rd  = c_inst->get_rs1p();
                id_issue_next.rs1 = c_inst->get_rs1p(); id_issue_next.rs2 = 0;
            ...
            case OP_C_ANDI:
                // ANDI rd', rd', imm
                id_issue_next.opcode = 0x13; id_issue_next.funct3 = 7;
                id_issue_next.rd  = c_inst->get_rs1p();
                id_issue_next.rs1 = c_inst->get_rs1p(); id_issue_next.rs2 = 0;
                id_issue_next.imm = static_cast<int64_t>(c_inst->get_imm_ADDI());
                break;
            case OP_C_SUB:
                // SUB rd', rd', rs2'
                id_issue_next.opcode = 0x33; id_issue_next.funct3 = 0; id_issue_next.funct7 = 0x20;
                id_issue_next.rd  = c_inst->get_rs1p();
                id_issue_next.rs1 = c_inst->get_rs1p();
                id_issue_next.rs2 = c_inst->get_rs2p();
                break;
            case OP_C_XOR:
                // XOR rd', rd', rs2'
                id_issue_next.opcode = 0x33; id_issue_next.funct3 = 4; id_issue_next.funct7 = 0;
                id_issue_next.rd  = c_inst->get_rs1p();
                id_issue_next.rs1 = c_inst->get_rs1p();
                id_issue_next.rs2 = c_inst->get_rs2p();
                break;
            case OP_C_OR:
                // OR rd', rd', rs2'
                id_issue_next.opcode = 0x33; id_issue_next.funct3 = 6; id_issue_next.funct7 = 0;
                id_issue_next.rd  = c_inst->get_rs1p();
                id_issue_next.rs1 = c_inst->get_rs1p();
                id_issue_next.rs2 = c_inst->get_rs2p();
                break;
            case OP_C_AND:
                // AND rd', rd', rs2'
                id_issue_next.opcode = 0x33; id_issue_next.funct3 = 7; id_issue_next.funct7 = 0;
                id_issue_next.rd  = c_inst->get_rs1p();
                id_issue_next.rs1 = c_inst->get_rs1p();
                id_issue_next.rs2 = c_inst->get_rs2p();
                break;
            case OP_C_SUBW:
                // SUBW rd', rd', rs2'  [RV64]
                id_issue_next.opcode = 0x3B; id_issue_next.funct3 = 0; id_issue_next.funct7 = 0x20;
                id_issue_next.rd  = c_inst->get_rs1p();
                id_issue_next.rs1 = c_inst->get_rs1p();
                id_issue_next.rs2 = c_inst->get_rs2p();
                break;
            case OP_C_ADDW:
                // ADDW rd', rd', rs2'  [RV64]
                id_issue_next.opcode = 0x3B; id_issue_next.funct3 = 0; id_issue_next.funct7 = 0;
                id_issue_next.rd  = c_inst->get_rs1p();
                id_issue_next.rs1 = c_inst->get_rs1p();
                id_issue_next.rs2 = c_inst->get_rs2p();
                break;
            ...
            case OP_C_BEQZ:
                // BEQ rs1', x0, offset
                id_issue_next.opcode = 0x63; id_issue_next.funct3 = 0;
                id_issue_next.rd  = 0;
                id_issue_next.rs1 = c_inst->get_rs1p(); id_issue_next.rs2 = 0;
                id_issue_next.imm = static_cast<int64_t>(
                    static_cast<int32_t>(c_inst->get_imm_CB()));
                break;
            case OP_C_BNEZ:
                // BNE rs1', x0, offset
                id_issue_next.opcode = 0x63; id_issue_next.funct3 = 1;
                id_issue_next.rd  = 0;
                id_issue_next.rs1 = c_inst->get_rs1p(); id_issue_next.rs2 = 0;
                id_issue_next.imm = static_cast<int64_t>(
                    static_cast<int32_t>(c_inst->get_imm_CB()));
            ...
            case OP_C_SLLI: {
                // SLLI rd, rd, shamt
                uint32_t shamt = (((ci >> 12) & 1) << 5) | ((ci >> 2) & 0x1F);
                id_issue_next.opcode = 0x13; id_issue_next.funct3 = 1;
                id_issue_next.rd  = c_inst->get_rs1();
                id_issue_next.rs1 = c_inst->get_rs1(); id_issue_next.rs2 = 0;
            ...
            case OP_C_JR:
                // JALR x0, 0(rs1)
                id_issue_next.opcode = 0x67; id_issue_next.funct3 = 0;
                id_issue_next.rd  = 0;
                id_issue_next.rs1 = c_inst->get_rs1(); id_issue_next.rs2 = 0;
                id_issue_next.imm = 0;
                break;
            case OP_C_MV:
                // ADD rd, x0, rs2
                id_issue_next.opcode = 0x33; id_issue_next.funct3 = 0; id_issue_next.funct7 = 0;
                id_issue_next.rd  = c_inst->get_rs1();
                id_issue_next.rs1 = 0;
            ...
            case OP_C_EBREAK:
                ...
                id_issue_next.opcode = 0x73; id_issue_next.funct3 = 0;
                id_issue_next.rd = 0; id_issue_next.rs1 = 0; id_issue_next.rs2 = 0;
                id_issue_next.imm = 1;
                break;
            case OP_C_JALR:
            ...
            case OP_C_SWSP:
                // SW rs2, offset(x2)
                id_issue_next.opcode = 0x23; id_issue_next.funct3 = 2;
                id_issue_next.rd  = 0; id_issue_next.rs1 = 2;
                id_issue_next.rs2 = c_inst->get_rs2();
            ...
            case OP_C_FSDSP:
                ...
                break;
            default:
                // Unrecognized or unsupported compressed instruction (e.g. C.FSDSP, C.FLD).
                // Mark as illegal so Issue_stage raises ILLEGAL_INSTR trap.
                id_issue_next.is_illegal = true;
                id_issue_next.valid = true;
                return;
        }
        ...
        // Correct branch predictor if it incorrectly predicted a non-branch compressed instruction as taken
        bool is_branch_instr = (id_issue_next.opcode == 0x63 || id_issue_next.opcode == 0x6F || id_issue_next.opcode == 0x67);
        if (!is_branch_instr && fetch_id_reg.predicted_taken) {
            pc_redirect_target = fetch_id_reg.pc + 2; // Compressed instructions are always 2 bytes
    ...
    if (id_issue_next.opcode == 0x37 || id_issue_next.opcode == 0x17 || id_issue_next.opcode == 0x6F) {
        ...
        id_issue_next.rs2 = 0;
               ...
               (id_issue_next.opcode == 0x73 && !(((instr >> 25) & 0x7F) == 0x09 && ((instr >> 12) & 0x7) == 0))) {
        id_issue_next.rs2 = 0;
    }
    ...
    // Correct branch predictor if it incorrectly predicted a non-branch instruction as taken
    bool is_branch_instr = (id_issue_next.opcode == 0x63 || id_issue_next.opcode == 0x6F || id_issue_next.opcode == 0x67);
    if (!is_branch_instr && fetch_id_reg.predicted_taken) {
...
void CPURV64P6_Cycle::Issue_stage() {
    // When EX is replaying a load (partial store-buffer overlap), freeze upstream.
    if (stall_ex) {
    ...
    // For M-extension: funct3 0-3 = multiply family, funct3 4-7 = divide/remainder family.
    FunctionalUnit fu = FunctionalUnit::ALU;
    {
        ...
        switch (id_issue_reg.opcode) {
            case 0x03: case 0x23: case 0x2F:  fu = FunctionalUnit::LSU;    break;
            case 0x63: case 0x6F: case 0x67:  fu = FunctionalUnit::BRANCH; break;
            case 0x73:                         fu = FunctionalUnit::CSR;    break;
            case 0x07: case 0x27:
            case 0x43: case 0x47: case 0x4B: case 0x4F:
            case 0x53:                         fu = FunctionalUnit::FPU;    break;
            default:
                if (is_m)
    ...
    // For each source register: if rd_clobber says it is busy, check whether the
    // producing scoreboard entry is already ready (EX ran before Issue this cycle).
    // If ready  → forward the result directly; no stall needed.
    // If not yet ready → stall until EX completes the producing instruction.
    ...
    bool reads_rs3 = (id_issue_reg.opcode == 0x43 || id_issue_reg.opcode == 0x47 || id_issue_reg.opcode == 0x4B || id_issue_reg.opcode == 0x4F);
    if (reads_rs3) {
    ...
    // CSR/FPU RAW hazard: if previous instruction wrote an FPU CSR (fcsr, frm, fflags) and this is an FPU instruction
    if (fu == FunctionalUnit::FPU) {
        if (pending_csr_write.valid && (pending_csr_write.addr == 0x001 || pending_csr_write.addr == 0x002 || pending_csr_write.addr == 0x003)) {
            need_stall = true;
        }
    }
    ...
    if ((fu == FunctionalUnit::MUL && mul_fu.busy) ||
        ...
        (fu == FunctionalUnit::LSU && (id_issue_reg.opcode == 0x03 || id_issue_reg.opcode == 0x2F) && dcache_miss_fu.busy)) {
        stall_issue = true;
        stall_fetch = true;
        stall_pcgen = true;
        issue_ex_next.valid = false;
    ...
    // AMOs require absolute memory atomicity and order. They must stall in Issue until
    // the store buffer has completely drained to memory to prevent reading or writing stale data.
    ...
    // is busy.  Once all FUs are idle, every prior ROB entry is guaranteed to have
    // ready=true.  When EX then fires the trap, the trap_pending drain loop:
    ...
    if (id_issue_reg.opcode == 0x73) {
        bool multi_cycle_in_flight = dcache_miss_fu.busy || mul_fu.busy ||
                                     div_fu.busy    || fpu_fu.busy;
    ...
    // store buffer, so is_store must be false to prevent a spurious commit_store() call.
    scoreboard[rob_idx].is_store  = (id_issue_reg.opcode == 0x23 || id_issue_reg.opcode == 0x27);
    scoreboard[rob_idx].is_branch = (id_issue_reg.opcode == 0x63 ||
                                     id_issue_reg.opcode == 0x6F ||
    ...
    if (id_issue_reg.is_illegal) {
        ...
        {
            uint64_t sb_addr, sb_data; int sb_size;
            while (store_buffer.drain_one(sb_addr, sb_data, sb_size)) {
                if (sb_size == 8) mem_intf->writeDataMem64(sb_addr, sb_data, sb_size);
                else              mem_intf->writeDataMem(sb_addr, static_cast<uint32_t>(sb_data), sb_size);
            }
        }
        scoreboard.flush();
        store_buffer.flush_speculative();
        mul_fu.busy = div_fu.busy = fpu_fu.busy = dcache_miss_fu.busy = load_hit_fu.busy = false;
        uint64_t tvec = csr.take_exception(CAUSE::ILLEGAL_INSTR, id_issue_reg.pc, id_issue_reg.instr);
    ...
    if (fu == FunctionalUnit::FPU) {
        ...
        if (id_issue_reg.opcode == 0x53) {
            uint32_t f7 = id_issue_reg.funct7;
            if (f7 == 0x50 || f7 == 0x51 || f7 == 0x60 || f7 == 0x61 || f7 == 0x70 || f7 == 0x71) {
...
void CPURV64P6_Cycle::EX_stage() {
    ...
    // --- Tick multi-cycle functional units (runs every cycle regardless of issue_ex_reg) ---
    // Each FU operates independently; the scoreboard handles data-dependent stalls.
    if (mul_fu.busy) {
    ...
    if (dcache_miss_fu.busy) {
        ...
        if (--dcache_miss_fu.remaining == 0) {
            ...
            scoreboard.complete(dcache_miss_fu.trans_id, dcache_miss_fu.result, dcache_miss_fu.rd);
    ...
    if (load_hit_fu.busy) {
        ...
        if (--load_hit_fu.remaining == 0) {
            scoreboard.complete(load_hit_fu.trans_id, load_hit_fu.result, load_hit_fu.rd);
    ...
    if (!issue_ex_reg.valid) return;

    // Guard: if cpu_process_IRQ already took control this cycle (flush_pipeline=true),
    // the ROB is flushed and sepc/irq_epc are set to the correct resume address.
    // Executing issue_ex_reg now would:
    //   (a) corrupt flushed ROB entries via scoreboard.complete()
    //   (b) call take_exception() if the instruction faults, overwriting the correct sepc
    // This is the root cause of the init SIGBUS: IRQ sepc=0x1014a (AUIPC) was being
    // overwritten by a load-page-fault sepc=0x1014e before the kernel ever saw 0x1014a.
    if (flush_pipeline) {
        return;
    }
    ...
    bool     multi_cycle_dispatched = false; // Set when MUL/DIV starts; skips scoreboard.complete()

    // Precise-exception guard: if the faulting instruction is not at the ROB head,
    // older in-flight instructions (e.g., a dcache-miss load) must commit first.
    // Stall EX and retry next cycle rather than firing the trap prematurely and
    // skipping those instructions (which would corrupt register state visible to
    // the trap handler, and cause SRET to resume past the skipped instructions).
    ...
    switch (issue_ex_reg.opcode) {
        case 0x33: // R-type (Register-Register)
            if (issue_ex_reg.funct7 == 0x01) {
                ...
                switch (issue_ex_reg.funct3) {
                    case 0x0: mul_result = static_cast<uint64_t>(s1 * s2); break;
                    case 0x1: mul_result = static_cast<uint64_t>(
                                  (static_cast<__int128>(s1) * s2) >> 64); break;
                    case 0x2: mul_result = static_cast<uint64_t>(
                                  (static_cast<__int128>(s1) *
                                   static_cast<unsigned __int128>(u2)) >> 64); break;
                    case 0x3: mul_result = static_cast<uint64_t>(
                                  (static_cast<unsigned __int128>(u1) * u2) >> 64); break;
                    case 0x4: // DIV (signed) — division by zero and overflow per spec
                        mul_result = (s2 == 0) ? static_cast<uint64_t>(-1LL)
                                   : (s1 == INT64_MIN && s2 == -1LL) ? static_cast<uint64_t>(s1)
                                   : static_cast<uint64_t>(s1 / s2);
                        break;
                    case 0x5: // DIVU (unsigned)
                        mul_result = (u2 == 0) ? UINT64_MAX : u1 / u2; break;
                    case 0x6: // REM (signed)
                        mul_result = (s2 == 0) ? static_cast<uint64_t>(s1)
                                   : (s1 == INT64_MIN && s2 == -1LL) ? 0
                                   : static_cast<uint64_t>(s1 % s2);
                        break;
                    case 0x7: // REMU (unsigned)
                        mul_result = (u2 == 0) ? u1 : u1 % u2; break;
                }
                bool is_div_op = (issue_ex_reg.funct3 >= 4);
                FunctionalUnitState& fu = is_div_op ? div_fu : mul_fu;
                fu.busy     = true;
                fu.remaining = is_div_op ? 63 : 1; // DIV=64 cycles, MUL=2 cycles
        ...
        case 0x1B: // OP-IMM-32: ADDIW / SLLIW / SRLIW / SRAIW
            switch (issue_ex_reg.funct3) {
                case 0x0: // ADDIW
                    result = static_cast<int64_t>(
                        static_cast<int32_t>(issue_ex_reg.rs1_val) +
                        static_cast<int32_t>(issue_ex_reg.imm));
                    break;
                case 0x1: // SLLIW
                    result = static_cast<int64_t>(static_cast<int32_t>(
                        static_cast<uint32_t>(issue_ex_reg.rs1_val) <<
            ...
            break;
        case 0x3B: // OP-32: ADDW/SUBW + M-extension W-type (MULW/DIVW/DIVUW/REMW/REMUW)
            if (issue_ex_reg.funct7 == 0x01) {
                ...
                switch (issue_ex_reg.funct3) {
                    ...
                    case 0x4: // DIVW — div-by-zero and INT32_MIN/-1 overflow per spec
                        w_result = (ws2 == 0) ? static_cast<int64_t>(-1)
                                 : (ws1 == INT32_MIN && ws2 == -1) ? static_cast<int64_t>(ws1)
                                 : static_cast<int64_t>(ws1 / ws2);
                        break;
                    case 0x5: // DIVUW
                        w_result = (wu2 == 0) ? static_cast<int64_t>(static_cast<int32_t>(-1))
                                 : static_cast<int64_t>(static_cast<int32_t>(wu1 / wu2));
                        break;
                    case 0x6: // REMW
                        w_result = (ws2 == 0) ? static_cast<int64_t>(ws1)
                                 : (ws1 == INT32_MIN && ws2 == -1) ? 0LL
                                 : static_cast<int64_t>(ws1 % ws2);
                        break;
                    case 0x7: // REMUW
                        w_result = (wu2 == 0) ? static_cast<int64_t>(static_cast<int32_t>(wu1))
                                 : static_cast<int64_t>(static_cast<int32_t>(wu1 % wu2));
                        break;
                }
            ...
            }
            {
                ...
                switch (issue_ex_reg.funct3) {
                    case 0x0: // ADDW / SUBW
                        result = static_cast<int64_t>(
                            (issue_ex_reg.funct7 == 0x20) ? (a - b) : (a + b));
                        break;
                    case 0x1: // SLLW
                        result = static_cast<int64_t>(
                            static_cast<int32_t>(ua << shamt));
                        break;
            ...
            break;
        case 0x6F: // JAL
            // Link address is pc+2 for compressed C.J (but rd=0, so it's discarded anyway)
        ...
        case 0x67: // JALR
            ...
            result = issue_ex_reg.pc + (issue_ex_reg.is_compressed ? 2u : 4u);
            branch_target = (issue_ex_reg.rs1_val + issue_ex_reg.imm) & ~static_cast<uint64_t>(1);
            branch_taken = true;
            break;
        case 0x63: // Branch
    ...
    if (issue_ex_reg.opcode == 0x03) { // Load
        ...
        uint64_t addr = vaddr; // declared before goto to avoid crossing initialization

        // Alignment check
        bool misaligned = false;
        switch (issue_ex_reg.funct3) {
            case 0x1: case 0x5: // halfword
                if (vaddr & 1) misaligned = true;
                break;
            case 0x2: case 0x6: // word
                if (vaddr & 3) misaligned = true;
                break;
            case 0x3: // doubleword
                if (vaddr & 7) misaligned = true;
                break;
        }
        if (misaligned) {
            ...
            if (tr_ua.fault) {
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(tr_ua.fault_cause, issue_ex_reg.pc, vaddr);
                pc_redirect_target = tvec; pc_redirect_valid = true;
                flush_pipeline = true; trap_pending = true;
            ...
            int ua_bytes = (issue_ex_reg.funct3 == 0x3) ? 8 :
                           ((issue_ex_reg.funct3 == 0x2 || issue_ex_reg.funct3 == 0x6) ? 4 : 2);
            uint64_t raw_ua = 0;
            for (int i = 0; i < ua_bytes; i++)
                raw_ua |= ((uint64_t)mem_intf->readDataMem(pa_ua + i, 1)) << (8 * i);
            switch (issue_ex_reg.funct3) {
                case 0x1: result = static_cast<int64_t>(static_cast<int16_t>(raw_ua)); break; // LH
                case 0x2: result = static_cast<int64_t>(static_cast<int32_t>(raw_ua)); break; // LW
                case 0x3: result = static_cast<int64_t>(raw_ua); break;                       // LD
                case 0x5: result = raw_ua & 0xFFFF; break;                                    // LHU
                case 0x6: result = raw_ua & 0xFFFFFFFF; break;                                // LWU
                default:  result = raw_ua; break;
            }
        ...
        // DTLB translation (Phase 8)
        {
            ...
            if (tr.fault) {
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(tr.fault_cause, issue_ex_reg.pc, vaddr);
                pc_redirect_target = tvec; pc_redirect_valid = true;
                flush_pipeline = true; trap_pending = true;
            ...
            if (tr.stall_cycles > 0) {
                ...
                dcache_miss_fu.result    = 0; // placeholder, overwritten below after real read
                dcache_miss_fu.trans_id  = issue_ex_reg.rob_index;
                dcache_miss_fu.rd        = issue_ex_reg.rd;
                multi_cycle_dispatched   = true;
                addr = tr.paddr;
                // Still perform the memory read so the result is ready when the stall expires.
                // (We overwrite dcache_miss_fu.result below.)
            } else {
                addr = tr.paddr;
            }
        }
        int load_size = 0;
        switch (issue_ex_reg.funct3) {
            case 0x0: case 0x4: load_size = 1; break;
            case 0x1: case 0x5: load_size = 2; break;
            case 0x2: case 0x6: load_size = 4; break;
            case 0x3:           load_size = 8; break;
        }

        if (!csr.pmp_check(addr, load_size, false, false)) {
            if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
            uint64_t tvec = csr.take_exception(CAUSE::LOAD_ACCESS, issue_ex_reg.pc, vaddr);
            pc_redirect_target = tvec; pc_redirect_valid = true;
            flush_pipeline = true; trap_pending = true;
            scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
            multi_cycle_dispatched = true;
        ...
        if (fwd_result == StoreBuffer<4,4>::FwdResult::HIT) {
            ...
            switch (issue_ex_reg.funct3) {
                case 0x0: fwd_result_val = static_cast<uint64_t>(static_cast<int64_t>(static_cast<int8_t>(fwd)));   break;
                case 0x1: fwd_result_val = static_cast<uint64_t>(static_cast<int64_t>(static_cast<int16_t>(fwd)));  break;
                case 0x2: fwd_result_val = static_cast<uint64_t>(static_cast<int64_t>(static_cast<int32_t>(fwd)));  break;
                case 0x3: fwd_result_val = fwd;                                               break;
                case 0x4: fwd_result_val = fwd & 0xFFULL;                                    break;
                case 0x5: fwd_result_val = fwd & 0xFFFFULL;                                  break;
                case 0x6: fwd_result_val = fwd & 0xFFFFFFFFULL;                              break;
            }
            ...
            // FwdResult::MISS — no overlapping stores in the buffer; safe to read memory.
            uint64_t mem_result = 0;
            switch (issue_ex_reg.funct3) {
                case 0x0: mem_result = static_cast<uint64_t>(static_cast<int64_t>(static_cast<int8_t>(mem_intf->readDataMem(addr, 1))));   break;
                case 0x1: mem_result = static_cast<uint64_t>(static_cast<int64_t>(static_cast<int16_t>(mem_intf->readDataMem(addr, 2))));  break;
                case 0x2: mem_result = static_cast<uint64_t>(static_cast<int64_t>(static_cast<int32_t>(mem_intf->readDataMem(addr, 4))));  break;
                case 0x3: mem_result = mem_intf->readDataMem64(addr, 8);                                                                    break;
                case 0x4: mem_result = mem_intf->readDataMem(addr, 1);                                                                     break;
                case 0x5: mem_result = mem_intf->readDataMem(addr, 2);                                                                     break;
                case 0x6: mem_result = mem_intf->readDataMem(addr, 4);                                                                     break;
            }
            if (multi_cycle_dispatched) {
                ...
                // D$ timing check: hit → result available this cycle; miss → defer via FU.
    ...
    } else if (issue_ex_reg.opcode == 0x23) { // Store → translate VA then add to speculative queue
        uint64_t vaddr_s = issue_ex_reg.rs1_val + issue_ex_reg.imm;
        uint64_t store_pa = vaddr_s; // declared before goto to avoid crossing initialization

        // Alignment check
        bool misaligned = false;
        switch (issue_ex_reg.funct3) {
            case 0x1: // halfword
                if (vaddr_s & 1) misaligned = true;
                break;
            case 0x2: // word
                if (vaddr_s & 3) misaligned = true;
                break;
            case 0x3: // doubleword
                if (vaddr_s & 7) misaligned = true;
                break;
        }
        if (misaligned) {
            ...
            if (tr_us.fault) {
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(tr_us.fault_cause, issue_ex_reg.pc, vaddr_s);
                pc_redirect_target = tvec; pc_redirect_valid = true;
                flush_pipeline = true; trap_pending = true;
                scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
                multi_cycle_dispatched = true;
            ...
            int us_bytes = (issue_ex_reg.funct3 == 0x3) ? 8 :
                           (issue_ex_reg.funct3 == 0x2 ? 4 : 2);
            uint64_t sval = issue_ex_reg.rs2_val;
            for (int i = 0; i < us_bytes; i++)
                mem_intf->writeDataMem(pa_us + i, static_cast<uint32_t>((sval >> (8*i)) & 0xFF), 1);
            scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
            goto ex_done;
        }

        {
            ...
            if (tr.fault) {
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(tr.fault_cause, issue_ex_reg.pc, vaddr_s);
                pc_redirect_target = tvec; pc_redirect_valid = true;
                flush_pipeline = true; trap_pending = true;
                scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
                multi_cycle_dispatched = true;
        ...
        if (!csr.pmp_check(store_pa, size, true, false)) {
            if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
            uint64_t tvec = csr.take_exception(CAUSE::STORE_ACCESS, issue_ex_reg.pc, vaddr_s);
            pc_redirect_target = tvec; pc_redirect_valid = true;
            flush_pipeline = true; trap_pending = true;
            scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
            multi_cycle_dispatched = true;
        ...
        store_buffer.add_store(store_pa, issue_ex_reg.rs2_val, size, issue_ex_reg.rob_index);
        if (load_reservation_valid && load_reservation_addr >= store_pa && load_reservation_addr < store_pa + size) {
            load_reservation_valid = false;
        }
    } else if (issue_ex_reg.opcode == 0x2F) { // AMO — Atomic Memory Operations (A extension)
        ...
        // VA -> PA translation.
        // LR is a load — use LOAD permission. SC and AMO RMW require STORE permission.
        {
            MMU::access_t amo_atype = (funct5 == 0x2) ? MMU::LOAD : MMU::STORE;
            auto tr = mmu->translate(vaddr_a, csr, amo_atype);
            if (tr.fault) {
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(tr.fault_cause, issue_ex_reg.pc, vaddr_a);
                pc_redirect_target = tvec; pc_redirect_valid = true;
                flush_pipeline = true; trap_pending = true;
                scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
                multi_cycle_dispatched = true;
        ...
        if (funct5 == 0x2) { // LR.W / LR.D
            ...
            if (lr_fwd == StoreBuffer<4,4>::FwdResult::HIT) {
                ...
                result = static_cast<int64_t>(static_cast<int32_t>(mem_intf->readDataMem(pa_a, 4)));
            }
            ...
            if (load_reservation_valid && load_reservation_addr == pa_a) {
                ...
                if (is_dword)
                    ...
                    mem_intf->writeDataMem(pa_a, static_cast<uint32_t>(issue_ex_reg.rs2_val), 4);
            ...
            load_reservation_valid = false;
        } else { // AMO read-modify-write: read old value, compute new, write directly (atomic)
            ...
            if (is_dword) {
                ...
                uint64_t new64;
                switch (funct5) {
                    case 0x1:  new64 = op64;  break;                                              // AMOSWAP
                    case 0x0:  new64 = old64 + op64; break;                                       // AMOADD
                    case 0x4:  new64 = old64 ^ op64; break;                                       // AMOXOR
                    case 0xC:  new64 = old64 & op64; break;                                       // AMOAND
                    case 0x8:  new64 = old64 | op64; break;                                       // AMOOR
                    case 0x10: new64 = (static_cast<int64_t>(old64) < static_cast<int64_t>(op64)) ? old64 : op64; break; // AMOMIN
                    case 0x14: new64 = (static_cast<int64_t>(old64) > static_cast<int64_t>(op64)) ? old64 : op64; break; // AMOMAX
                    case 0x18: new64 = (old64 < op64) ? old64 : op64; break;                      // AMOMINU
                    case 0x1C: new64 = (old64 > op64) ? old64 : op64; break;                      // AMOMAXU
                    default:   new64 = old64; break;
                }
                ...
                uint32_t new32;
                switch (funct5) {
                    case 0x1:  new32 = op32;  break;                                              // AMOSWAP.W
                    case 0x0:  new32 = old32 + op32; break;                                       // AMOADD.W
                    case 0x4:  new32 = old32 ^ op32; break;                                       // AMOXOR.W
                    case 0xC:  new32 = old32 & op32; break;                                       // AMOAND.W
                    case 0x8:  new32 = old32 | op32; break;                                       // AMOOR.W
                    case 0x10: new32 = (static_cast<int32_t>(old32) < static_cast<int32_t>(op32)) ? old32 : op32; break; // AMOMIN.W
                    case 0x14: new32 = (static_cast<int32_t>(old32) > static_cast<int32_t>(op32)) ? old32 : op32; break; // AMOMAX.W
                    case 0x18: new32 = (old32 < op32) ? old32 : op32; break;                      // AMOMINU.W
                    case 0x1C: new32 = (old32 > op32) ? old32 : op32; break;                      // AMOMAXU.W
                    default:   new32 = old32; break;
                }
                ...
                store_buffer.invalidate_address(pa_a, amo_size);
                load_reservation_valid = false;
                result = static_cast<int64_t>(static_cast<int32_t>(old32)); // sign-extend to 64-bit
            }
        }
    }

    // 3. Branch Prediction Verification & Training
    bool is_branch_instruction; // declared without initializer to allow goto from above
    is_branch_instruction = (issue_ex_reg.opcode == 0x63 || issue_ex_reg.opcode == 0x6F || issue_ex_reg.opcode == 0x67);
    
    if (is_branch_instruction) {
        // Alignment check
        if (branch_taken && (branch_target & 1)) {
            if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
            uint64_t tvec = csr.take_exception(CAUSE::INSTR_MISALIGN, issue_ex_reg.pc, branch_target);
            pc_redirect_target = tvec; pc_redirect_valid = true;
            flush_pipeline = true; trap_pending = true;
            scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
            multi_cycle_dispatched = true;
        ...
        if (mispredict) {
             pc_redirect_target = branch_taken ? branch_target : (issue_ex_reg.pc + (issue_ex_reg.is_compressed ? 2 : 4));
             pc_redirect_valid = true;
             flush_pipeline = true;
             stats.flushes++;
             stats.branch_mispredicts++;
    ...
    if (issue_ex_reg.opcode == 0x73) {
        ...
        if (f3 == 0) {
            ...
            if (csr_addr == 0x000) {
                ...
                uint64_t cause;
                switch (csr.priv) {
                    case PrivMode::U: cause = CAUSE::ECALL_U; break;
                    case PrivMode::S: cause = CAUSE::ECALL_S; break;
                    default:          cause = CAUSE::ECALL_M; break;
                }
                ...
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(CAUSE::BREAKPOINT, issue_ex_reg.pc, 0);
                pc_redirect_target = tvec;
                pc_redirect_valid  = true;
                flush_pipeline     = true;
                trap_pending       = true;
                ...
                if (csr.priv != PrivMode::M) {
                    uint64_t tvec = csr.take_exception(CAUSE::ILLEGAL_INSTR, issue_ex_reg.pc, issue_ex_reg.instr);
                    pc_redirect_target = tvec; pc_redirect_valid = true; flush_pipeline = true;
                } else {
                ...
                // Check for SFENCE.VMA: funct7=0x09, funct3=0, rs1/rs2 in instruction fields
                // Encoded as: imm[31:20]=0b000100101XX where bits[24:20]=rs2, bits[19:15]=rs1
                ...
                if (f7 == 0x09) {
                    ...
                    mmu->sfence_vma(issue_ex_reg.rs1_val, issue_ex_reg.rs2_val, rs1_zero, rs2_zero);
                    flush_pipeline = true; // Redirect to PC+4 to drain pipeline
                    pc_redirect_target = issue_ex_reg.pc + 4;
                    pc_redirect_valid  = true;
                }
                // Other unknown funct12 — treat as NOP (future: illegal instruction trap)
            }
        } else {
            ...
            if (!ok) {
                ...
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                trap_vector_addr = csr.take_exception(CAUSE::ILLEGAL_INSTR, issue_ex_reg.pc, issue_ex_reg.instr);
                trap_pending     = true;
                flush_pipeline   = true;
                pc_redirect_target = trap_vector_addr;
                pc_redirect_valid  = true;
    ...
    if (scoreboard[issue_ex_reg.rob_index].fu == FunctionalUnit::FPU) {
        ...
        bool is_double = (issue_ex_reg.opcode == 0x07 && issue_ex_reg.funct3 == 3) ||
                          (issue_ex_reg.opcode == 0x27 && issue_ex_reg.funct3 == 3) ||
                          (issue_ex_reg.opcode != 0x07 && issue_ex_reg.opcode != 0x27 && ((issue_ex_reg.instr >> 25) & 0x3) == 1);
        ...
        if (issue_ex_reg.opcode == 0x07) { // Load-FP
            ...
            if (misaligned) {
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(CAUSE::LOAD_MISALIGN, issue_ex_reg.pc, vaddr);
                pc_redirect_target = tvec; pc_redirect_valid = true;
                flush_pipeline = true; trap_pending = true;
                scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
                multi_cycle_dispatched = true;
            ...
            if (misaligned) {
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(CAUSE::STORE_MISALIGN, issue_ex_reg.pc, vaddr);
                pc_redirect_target = tvec; pc_redirect_valid = true;
                flush_pipeline = true; trap_pending = true;
                scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
                multi_cycle_dispatched = true;
            ...
            if (!csr.pmp_check(pa, is_double ? 8 : 4, true, false)) {
                if (has_unready_older()) { stall_ex = true; multi_cycle_dispatched = true; goto ex_done; }
                uint64_t tvec = csr.take_exception(CAUSE::STORE_ACCESS, issue_ex_reg.pc, vaddr);
                pc_redirect_target = tvec; pc_redirect_valid = true;
                flush_pipeline = true; trap_pending = true;
                scoreboard.complete(issue_ex_reg.rob_index, 0, 0);
                multi_cycle_dispatched = true;
        ...
        if (issue_ex_reg.is_compressed) {
            if (issue_ex_reg.opcode == 0x07) {
                instr_to_decode = ((issue_ex_reg.imm & 0xFFF) << 20) | (issue_ex_reg.rs1 << 15) | (issue_ex_reg.funct3 << 12) | (issue_ex_reg.rd << 7) | 0x07;
                ...
                instr_to_decode = (imm_11 << 25) | (issue_ex_reg.rs2 << 20) | (issue_ex_reg.rs1 << 15) | (issue_ex_reg.funct3 << 12) | (imm_5 << 7) | 0x27;
            }
        }
        ...
        if (f_op != OP_F_ERROR) {
            latency = f_inst->execute(f_op, issue_ex_reg.rs1_val, issue_ex_reg.rs2_val, 0, issue_ex_reg.rd, pa, fcsr_flags);
            if (writes_to_int_reg) {
                fpu_result = register_bank->getValue(issue_ex_reg.rd);
            }
        } else if (d_op != OP_D_ERROR) {
            latency = d_inst->execute(d_op, issue_ex_reg.rs1_val, issue_ex_reg.rs2_val, 0, issue_ex_reg.rd, pa, fcsr_flags);
            if (writes_to_int_reg) {
                fpu_result = register_bank->getValue(issue_ex_reg.rd);
            }
        } else {
    ...
    //    Multi-cycle FUs (MUL/DIV/D$-miss) call complete() themselves when they finish.
    ex_done:
    ...
    if (trap_pending) {
        ...
        // trap handler.  Drain ready ROB entries from head up to (but not including)
        // the trapping instruction's rob_index.
        //
...
void CPURV64P6_Cycle::Commit_stage() {
    ...
    // independent of whether a new instruction is committing.
    {
        ...
        if (store_buffer.drain_one(addr, data, size)) {
            ...
            rs.pa = addr; rs.data = data; rs.size = size;
            rs.pc = pc_current; rs.cycle = stats.cycles;
            recent_store_idx++;
        }
    }
...
void CPURV64P6_Cycle::printStats() const {
    ...
    std::cout << "  Forwarded:    " << stats.forwarded_reads     << "  (RAW resolved by fwd)\n";
    std::cout << "  LD/ST Fwd:    " << stats.load_store_forwards << "  (loads from store buf)\n";
    std::cout << "  MUL busy:     " << stats.mul_stall_cycles    << "  (cycles multiplier active)\n";
    std::cout << "  DIV busy:     " << stats.div_stall_cycles    << "  (cycles divider active)\n";
    {
    ...
    if (mmu) {
        ...
        if (it > 0)
            std::cout << "  ITLB miss rt: " << std::fixed << std::setprecision(2)
                      << (100.0 * ms.itlb_misses / it) << "%  (" << ms.itlb_misses << " misses, "
                      << stats.itlb_miss_cycles << " stall cycles)\n";
        if (dt > 0)
            std::cout << "  DTLB miss rt: " << std::fixed << std::setprecision(2)
                      << (100.0 * ms.dtlb_misses / dt) << "%  (" << ms.dtlb_misses << " misses, "
                      << stats.dtlb_miss_cycles << " stall cycles, "
...
void CPURV64P6_Cycle::dumpPipelineTrace(const std::string& filename) const {
    ...
    // CSV Rows
    for (const auto& e : pipeline_trace) {
        ofs << e.cycle << ","
