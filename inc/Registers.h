...
#include <cstdio>
extern uint64_t global_cpu_cycle;
 ...
 */
    ...
    public:
        ...
        uint64_t getFPValue(int reg) const { return f_regs[reg & 0x1F]; }
        void     setFPValue(int reg, uint64_t val) { f_regs[reg & 0x1F] = val; }

        float getF32(int reg) const {
            uint32_t bits = static_cast<uint32_t>(f_regs[reg & 0x1F] & 0xFFFFFFFF);
            float v; std::memcpy(&v, &bits, 4); return v;
        }
        void setF32(int reg, float v) {
            uint32_t bits; std::memcpy(&bits, &v, 4);
            f_regs[reg & 0x1F] = 0xFFFFFFFF00000000ULL | bits; // NaN-box
        }
        double getF64(int reg) const {
            double v; std::memcpy(&v, &f_regs[reg & 0x1F], 8); return v;
        }
        void setF64(int reg, double v) {
            std::memcpy(&f_regs[reg & 0x1F], &v, 8);
        }
