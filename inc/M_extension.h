 ...
 */
    ...
    public:
        ...
        void Exec_M_DIVUW() const {
            ...
            // DIVUW is unsigned — no overflow case exists (unlike signed DIVW).
            // Only special case per RISC-V spec: divide-by-zero returns all-ones,
            // sign-extended to 64 bits as 0xFFFFFFFFFFFFFFFF.
            if (divisor == 0) {
                result = -1LL; // all-ones sign-extended into int64_t result
            } else {
                // Quotient is uint32_t; name the int32_t step explicitly so the
                // compiler resolves the type without dependent-base lookup warnings.
                std::int32_t sq = static_cast<std::int32_t>(dividend / divisor);
                result = sq; // implicit widening sign-extends to int64_t
            }
